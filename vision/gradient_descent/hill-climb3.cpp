...
line_segment_all_frames get_visible_view_line(line_segment_3d world_line,
											   ...
											   float near_dist = 1.0)
{
	 ...
	 * world_line (a 3d line segment) with the given particle_camera pose
	 *
	 * Arguments:
	 * -------------------------------------------------------------
	 * world_line	   : The 3d Line which needs to be projected using
	 * 					 the camera pose
	...
	Point3d cam_world_pt2 = get_camera_world_coordinates(
	    ...
	    {
	    	cout<<"Exception 42"<<endl;
	    	throw 42; // indicates that line is outside the camera view
	    }
		...
		{
			cout<<"Exception 43"<<endl;
			throw 43; // skip the line if both the projection
														 ...
														 float near_dist = 1.0)
{
	...
	for(unsigned int k = 0; k < court.court_lines.size(); k++)
	{
		...
		try
		{
			line_seg = get_visible_view_line(court.court_lines[k],
											  particle_camera,
				                              frame_size,
											  view_frame,
											  near_dist);
			line_seg.line_number = k;
														 ...
														 float near_dist = 0.2)
{
	...
	{
		line_segment_all_frames line_seg;
		line_seg = get_extended_view_line(court.court_lines[k],
										   particle_camera,
				                           frame_size,
										   view_frame, near_dist);
		line_seg.line_number = k;
		view_lines.push_back(line_seg);
	}
	return view_lines;
...
int match_lines(int line_number, vector<line_segment_all_frames> actual_view)
{
	/* Returns the index of the line in the array (actual_view)
	 * which matches with the line_number.
	 */

	for(unsigned int k= 0; k< actual_view.size(); k++)
		if (actual_view[k].line_number == line_number)
			return k;
...
////---------- DRAW the actual and the particle views-------------------------------------
	// Draw the actual_view
	for(unsigned int j = 0; j < actual_view.size(); j++)
	{
		...
		line(actual_frame, actual_view[j].imgPlane.pt1,actual_view[j].imgPlane.pt2, color, 2, 8);
	}

		for(unsigned int k= 0; k < actual_view.size(); k++)
			{
				int line_number = actual_view[k].line_number;

				line_segment_all_frames particle_view_line = get_extended_view_line(
												court.court_lines[line_number],
												particle_camera,
												particle_frame.size(),
												particle_frame);
				particle_view.push_back(particle_view_line);
			}

		for(unsigned int j = 0; j < particle_view.size(); j++)
		{
			Scalar color = Scalar(255*rng.uniform((double)0, (double)1),
										  255*rng.uniform((double)0, (double)1),
										  255*rng.uniform((double)0, (double)1));
			if(!particle_view[j].behind_camera)
...
//-------------------------------------------------------------------------------
	...
	while(num_iters<100000)
	{
		...
		for(int i = 1; i<=5; i++)
			for(unsigned int k= 0; k < actual_view.size(); k++)
			{
				...
				double slope;
				if(particle_view_line.behind_camera)
				{
					slope = differentiate_zPull(particle_view_line.realWorld,
												particle_view_line.camWorld,
												i, particle_camera,
												vars);
				}
				else
				{
					slope = differentiate_costFunction_by_cameraParameter(
											particle_view_line.imgPlane,
											particle_view_line.realWorld,
											particle_view_line.camWorld,
											actual_view[k].imgPlane, i,
											particle_camera, vars);
				}

				if(slope == slope) //check for a valid double value
					slopes[i-1] += slope;
			}
	...
	// Draw the actual_view
	actual_frame  = Mat::zeros(480, 640, CV_8UC3); // clear the frame
	for(unsigned int j = 0; j < actual_view.size(); j++)
	{
		Scalar color = Scalar(255*rng.uniform((double)0, (double)1),
								  255*rng.uniform((double)0, (double)1),
...
////---------- DRAW the particle's view-------------------------------------
	particle_frame  = Mat::zeros(480, 640, CV_8UC3); // clear the frame
	for(unsigned int k= 0; k < actual_view.size(); k++)
		{
			int line_number = actual_view[k].line_number;
	...
	cout<<"Particle lines #: "<<particle_view.size()<<endl;
	for(unsigned int j = 0; j < particle_view.size(); j++)
	{
		Scalar color = Scalar(255*rng.uniform((double)0, (double)1),
									  255*rng.uniform((double)0, (double)1),
...
void update_view_on_trackbar_change(int, void* )
{
	...
	cout<<"Before: "<<endl;
			cout<<"    Delta X: "<<particle_cam.position.x - bb_camera.position.x
			    <<" | Delta Y: "<<particle_cam.position.y - bb_camera.position.y
			    <<" | Delta Z: "<<particle_cam.position.z - bb_camera.position.z
			    <<endl
			    <<"    Delta Pan: "<<((particle_cam.theta+particle_cam.pan -
						  bb_camera.theta - bb_camera.pan)*180.0/pi)
			    <<" | Delta Tilt: "<<((particle_cam.tilt - bb_camera.tilt)*180.0/pi)
			    <<endl;

	hill_climb(actual_view,
