...
if [ $HOSTTYPE == "x86_64" ]; then
	...
	gcc -c -fPIC BlobResult.cpp BlobResult_wrap.cxx BlobExtraction.cpp Blob.cpp Blob_wrap.cxx -I/usr/include/python2.6 `pkg-config --cflags opencv`
else
	echo "You are not 64-bit."
	gcc -c BlobResult.cpp BlobResult_wrap.cxx BlobExtraction.cpp Blob.cpp Blob_wrap.cxx -I/usr/include/python2.6 `pkg-config --cflags opencv`
fi
